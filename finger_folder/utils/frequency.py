...
def ridge_freq(im, mask, orient, block_size, kernel_size, minWaveLength, maxWaveLength):
    ...
    print(mask)
    print("angle≠0 비율:",
      np.count_nonzero(orient) / orient.size * 100, "%")
    test_block = im[0:block_size, 0:block_size]
    ridge_sum  = np.sum(test_block, axis=0)
    import matplotlib.pyplot as plt; plt.plot(ridge_sum); plt.show()
    medianfreq = np.median(non_zero_elems_in_freq) if non_zero_elems_in_freq.size else 0
    if medianfreq == 0:
        medianfreq = 0.1      # 주파수 0.1 ≒ 능선 간격 10 px
    medianfreq = medianfreq * mask
